...
if (isset($_SESSION['email'])) {
    ...
    if ($rowUserId = mysqli_fetch_assoc($resultUserId)) {
        ...
        $query = "SELECT * FROM driverinfo WHERE driver_owner = $loggedInUserId";
        $result = mysqli_query($conn, $query);

        $twoMonthsFromNow = date('Y-m-d', strtotime('+2 months'));
        while ($row = mysqli_fetch_assoc($result)) {
            if (strtotime($row['dri_licenseExpiry']) <= strtotime($twoMonthsFromNow)) {
                if ($row['email_sent'] == 0) {
                    ...
                    $emailBody = "";
                    $emailBody = "Driver Name: ". $row['dri_name'] . "\n";
                    $emailBody .= "Driver Registration Number: " . $row['dri_regnumber'] . "\n";
                    $emailBody .= "Driver License Expiry: " . $row['dri_licenseExpiry'] . "\n\n";
        
                    $mail->Body = "Hello, the following drivers have licenses about to expire:\n\n" . $emailBody;
        
                    if ($mail->send()) {
                        $driverId = $row['driver_id'];
                        $updateQuery = "UPDATE driverinfo SET email_sent = 1 WHERE driver_id = $driverId";
        ...
        <div class="col">
            <div class="card mt-5">
                <div class="card-header" style="background-color: #d6d46d">
                    <h2 class="display-6 text-center">Registered Drivers</h2>
                </div>
                <div class="card-body">
                <table class="table table-bordered text-center">
                    <tr class="bg-dark text-white"></tr>
                        ...
                        ?>
                        <tr>
                        <td><?php echo $row['dri_regnumber']?></td>
                        <td><?php echo $row['dri_name']?></td>
                        <td><?php echo $row['dri_licensenumber']?></td>
                        <td><?php echo $row['dri_joiningDate']?></td>
                        <td><?php echo $row['dri_licenseExpiry']?></td>
                        <td><a href="viewProfile.php?vid=<?php echo $row['driver_id'];?>">View</a></td>
                        <td><a href="edit.php?edid=<?php echo $row['driver_id'];?>">Edit</a></td>
                        <td><a href="delete.php?delid=<?php echo $row['driver_id'];?>">Delete</a></td>
                        <td>
                            <?php
                            if (strtotime($row['dri_licenseExpiry']) <= strtotime($twoMonthsFromNow)) {
